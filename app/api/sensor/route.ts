...
export async function POST(req: Request) {
  try {
    ...
    if (typeof temperature !== "number") return NextResponse.json({ error: "Invalid temperature" }, { status: 400 });
    if (typeof humidity !== "number") return NextResponse.json({ error: "Invalid humidity" }, { status: 400 });
